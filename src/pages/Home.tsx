...
// Inline SVG logo mark — used inside html2canvas card (no external file needed)
  ...
  const generateLoadingQuote = (
    ...
    if (sessions.length >= 3) {
      ...
      if (last7Days.length >= 3) {
        dataQuotes.push({
          text: `${last7Days.length} workouts in the last 7 days.`,
          sub: "That's serious consistency. Keep it up.",
        });
      }
    }
    ...
    if (body.length >= 2) {
      ...
      if (wCur != null && wPrev != null) {
        ...
        if (diff < -0.3) {
          dataQuotes.push({
            text: `Down ${Math.abs(diff).toFixed(1)}kg since your last check-in.`,
            sub: "The scale is moving. Stay the course.",
          });
        } else if (diff > 0.3) {
          dataQuotes.push({
            text: `Up ${diff.toFixed(1)}kg — could be muscle, could be water.`,
            sub: "Trust the process. Keep tracking.",
          });
        }
      }
      const pbfCur = cur.pbf != null ? Number(cur.pbf) : null;
      const pbfPrev = prev.pbf != null ? Number(prev.pbf) : null;
      if (pbfCur != null && pbfPrev != null && pbfCur < pbfPrev) {
        dataQuotes.push({
          text: `Body fat down ${(pbfPrev - pbfCur).toFixed(1)}% since last entry.`,
          sub: "Fat loss is happening. Keep going.",
        });
      }
      const smmCur = cur.smm != null ? Number(cur.smm) : null;
      const smmPrev = prev.smm != null ? Number(prev.smm) : null;
      if (smmCur != null && smmPrev != null && smmCur > smmPrev) {
        dataQuotes.push({
          text: `Muscle mass up ${(smmCur - smmPrev).toFixed(1)}kg.`,
          sub: "You're building strength. Don't stop now.",
        });
      }
    }
    ...
    if (sessions.length >= 10) {
      dataQuotes.push({
        text: `${sessions.length} workouts logged so far.`,
        sub: "Every single one of them mattered.",
      });
    }
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        setLoadingQuote(generateLoadingQuote(sessions, bodyStatsData));
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (navigator.share && navigator.canShare({ files: [file] })) {
          try { await navigator.share({ files: [file], title: 'FitTrack' }); } catch {}
        } else {
          const url = URL.createObjectURL(blob);
          const a = document.createElement('a');
          a.href = url;
          a.download = 'fittrack-quote.png';
          a.click();
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center px-8">
        ...
        <div className="mb-10">
          <svg width="64" height="64" viewBox="0 0 64 64">
            ...
            <circle cx="32" cy="32" r="28" fill="none" stroke="#10b981" strokeWidth="6"
              ...
              style={{ animation: 'ftspin 1.4s linear infinite', transformOrigin: '32px 32px' }}/>
            <circle cx="32" cy="32" r="18" fill="none" stroke="rgba(52,211,153,0.12)" strokeWidth="5"/>
            <circle cx="32" cy="32" r="18" fill="none" stroke="#34d399" strokeWidth="5"
              strokeDasharray="113" strokeDashoffset="56"
              strokeLinecap="round" transform="rotate(-90 32 32)"
              style={{ animation: 'ftspin 1.8s linear infinite reverse', transformOrigin: '32px 32px' }}/>
          </svg>
          <style>{`@keyframes ftspin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }`}</style>
        </div>

        {/* Branded quote card — screenshotted by html2canvas */}
        {loadingQuote ? (
          <div
            ref={quoteCardRef}
            style={{
          ...
          >
            {/* Inline logo mark — no external file, html2canvas safe */}
            <FitTrackLogoMark />
            ...
            <p style={{ color: '#10b981', fontSize: '13px', fontWeight: 500, marginBottom: 0 }}>
              {loadingQuote.sub}
            </p>
            ...
            <div style={{
              ...
              <p style={{ color: 'rgba(100,116,139,0.7)', fontSize: '10px', letterSpacing: '0.05em', margin: 0 }}>
        ...
        {loadingQuote && (
          <button
            ...
            className="mt-6 flex items-center gap-2 px-5 py-2.5 bg-slate-800 border border-slate-700 hover:border-emerald-500/40 rounded-full text-slate-400 hover:text-emerald-400 text-xs transition-colors"
          >
        ...
        <div className="absolute bottom-10 text-slate-700 text-[10px] font-mono tracking-widest uppercase">
          FitTrack
        </div>
      </div>
    );
      ...
      <div className="p-5 space-y-4">
        ...
        {totalComplete < 3 && (
          <div className="bg-slate-900 border border-blue-500/20 rounded-xl p-3">
            ...
            <div className="h-1 bg-slate-800 rounded-full mb-3 overflow-hidden">
              <div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${Math.round((totalComplete / 3) * 100)}%` }} />
            </div>
            <div className="grid grid-cols-3 gap-2">
              {([
                ...
                return (
                  <div key={key} onClick={() => !done && navigate(path)}
                    className={`text-center py-2 px-1 rounded-lg border transition-colors ${done ? 'bg-blue-500/10 border-blue-500/30 cursor-default' : 'bg-slate-800 border-slate-700 cursor-pointer hover:border-blue-500/40'}`}>
              ...
              <div key={day.dateStr} className="flex flex-col items-center gap-1">
                <svg width="28" height="28" viewBox="0 0 28 28" style={{ opacity: day.isFuture ? 0.2 : 1 }}>
                  ...
                  <circle cx="14" cy="14" r="11.5" fill="none" stroke="#ff375f" strokeWidth="4"
                    strokeDasharray={arc(11.5, day.trainVal).dasharray} strokeDashoffset={arc(11.5, day.trainVal).dashoffset}
                  ...
                  <circle cx="14" cy="14" r="8" fill="none" stroke="#30d158" strokeWidth="3.5"
                    strokeDasharray={arc(8, day.moveVal).dasharray} strokeDashoffset={arc(8, day.moveVal).dashoffset}
                    strokeLinecap="round" transform="rotate(-90 14 14)"/>
                  ...
                  <circle cx="14" cy="14" r="5" fill="none" stroke="#32ade6" strokeWidth="3"
                    strokeDasharray={arc(5, day.trackVal).dasharray} strokeDashoffset={arc(5, day.trackVal).dashoffset}
                    strokeLinecap="round" transform="rotate(-90 14 14)"/>
                  ...
                  <circle cx="14" cy="14" r="2.5" fill="none" stroke="#f97316" strokeWidth="2.5"
                    strokeDasharray={arc(2.5, day.fuelVal).dasharray} strokeDashoffset={arc(2.5, day.fuelVal).dashoffset}
                    strokeLinecap="round" transform="rotate(-90 14 14)"/>
                </svg>
        ...
        {upcomingTests.length > 0 && (
          <div onClick={() => navigate('/labs')}
            ...
            <span className="text-amber-400 text-sm">🔔 {upcomingTests.length} lab test{upcomingTests.length > 1 ? 's' : ''} due soon</span>
        ...
        {muscleAlert && (
          <div className="bg-amber-500/10 border border-amber-500/25 rounded-xl px-4 py-3">
            <span className="text-amber-400 text-sm">⚠️ {muscleAlert.group} day — {muscleAlert.daysSince} days since last session</span>
          </div>
        )}
            ...
            <div className="flex gap-2">
              {[
                ...
                <span key={status} className={`px-2.5 py-0.5 rounded-full text-[10px] font-medium border ${
                  (status === 'improving' || status === 'strong') ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30'
                  : (status === 'hold' || status === 'steady') ? 'bg-amber-500/20 text-amber-400 border-amber-500/30'
              ...
              <div className={`${insightColors[currentTopic].bg} border ${insightColors[currentTopic].border} rounded-xl p-4`}>
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => currentTopic === 'food' ? navigate('/food')
                        : currentTopic === 'workout' ? navigate('/workouts')
                        : currentTopic === 'labs' ? navigate('/labs')
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => navigate('/wellness')} className="text-[10px] font-mono text-blue-400">Wellness →</button>
          </div>
              ...
              return (
                <div className={`rounded-xl border px-3 py-2.5 flex items-center gap-3 ${waterCount > 0 ? 'bg-blue-500/8 border-blue-500/20' : 'bg-slate-900 border-slate-800'}`}>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => { const v = Math.max(0, waterCount - 1); saveWater(v); }} className="w-6 h-6 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 text-xs font-bold flex items-center justify-center">−</button>
                    <span className="text-xs font-mono text-white w-4 text-center">{waterCount}</span>
                    <button onClick={() => saveWater(waterCount + 1)} className="w-6 h-6 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 text-xs font-bold flex items-center justify-center">+</button>
              ...
              return (
                <div className={`rounded-xl border px-3 py-2.5 flex items-center gap-3 ${sleepHours > 0 ? 'bg-indigo-500/8 border-indigo-500/20' : 'bg-slate-900 border-slate-800'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-0.5 bg-slate-800 rounded-full mt-1 overflow-hidden">
                      <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, backgroundColor: isGood ? '#34d399' : '#818cf8' }} />
                    </div>
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => { const v = Math.max(0, parseFloat((sleepHours - 0.5).toFixed(1))); setSleepHours(v); setSleepInput(String(v)); }} className="w-6 h-6 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 text-xs font-bold flex items-center justify-center">−</button>
                    ...
                    <button onClick={() => { const v = parseFloat((sleepHours + 0.5).toFixed(1)); setSleepHours(v); setSleepInput(String(v)); }} className="w-6 h-6 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 text-xs font-bold flex items-center justify-center">+</button>
                    <button onClick={() => { const v = Math.max(0, Math.min(24, parseFloat(sleepInput) || 0)); setSleepInput(String(v)); saveSleep(v); }} className="w-6 h-6 bg-indigo-500/15 border border-indigo-500/30 hover:bg-indigo-500/30 rounded-lg text-indigo-400 text-xs flex items-center justify-center transition-colors">✓</button>
            ...
            {(() => {
              ...
              return (
                <div className={`rounded-xl border px-3 py-2.5 flex items-center gap-3 ${stepsCount > 0 ? 'bg-green-500/8 border-green-500/20' : 'bg-slate-900 border-slate-800'}`}>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => { const v = Math.max(0, stepsCount - 1000); setStepsInput(String(v)); saveSteps(v); }} className="w-6 h-6 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 text-xs font-bold flex items-center justify-center">−</button>
                    ...
                    <button onClick={() => { const v = stepsCount + 1000; setStepsInput(String(v)); saveSteps(v); }} className="w-6 h-6 bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400 text-xs font-bold flex items-center justify-center">+</button>
                    <button onClick={() => { const v = Math.max(0, parseInt(stepsInput) || 0); setStepsInput(String(v)); saveSteps(v); }} className="w-6 h-6 bg-green-500/15 border border-green-500/30 hover:bg-green-500/30 rounded-lg text-green-400 text-xs flex items-center justify-center transition-colors">✓</button>
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
          ...
          <button
            onClick={() => suggestedWorkout === 'Running' ? navigate('/running-session')
